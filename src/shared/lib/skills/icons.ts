...
export const getSkillIconUrl = (skillName: string): string | null => {
  ...
  if (skillIconId) {
    const url = `https://skillicons.dev/icons?i=${skillIconId}`;
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Skill Icon] ${skillName} → ${url}`);
    }
    return url;
  }
  ...
  if (!simpleIconId) {
    if (process.env.NODE_ENV === 'development') {
      console.warn(`[Skill Icon] ${skillName} → 아이콘을 찾을 수 없습니다.`);
    }
    return null;
  }

  // Simple Icons CDN URL (색상 없음, CSS로 색상 적용 필요)
  const url = `https://cdn.jsdelivr.net/npm/simple-icons@v9/icons/${simpleIconId}.svg`;
  if (process.env.NODE_ENV === 'development') {
    console.log(`[Skill Icon] ${skillName} → ${url}`);
  }
  return url;
